...
| **P4.2** | Auth: sesiones JWT propias (jose+bcryptjs), registro/login/logout, protección de rutas | login/registro funcional, sesión persistente | ✅ hecho (commit `717d3da`, 22 tests) |
| **P4.3** | Salas y mensajes: crear/entrar salas, historial desde BD, enviar mensajes por Socket.io y persistir | dos clientes conversando en tiempo real + historial al entrar | ✅ hecho (commit `93ffcb6`, 29 tests, verificado con 2 clientes) |
| **P4.4** | Tiempo real avanzado: "escribiendo…" y presencia de usuarios (online/offline) | indicador de escritura y lista de presentes por sala | ✅ hecho (commit `7e9299b`, 33 tests, verificado en vivo) |
| **P4.5** | Cierre: README, CI (GitHub Actions), deploy Railway, screenshot, subir al portafolio (`content.ts` ES/EN) | P4 en el portafolio + CI ✅ + 4 proyectos | ✅ hecho (commit `a1fe80c`, CI run #1 ✅; deploy y screenshot pendientes) |
...
| 2026-08-12 | **P4.2 Chat: auth completado** | Auth de sesiones JWT propias en `chat/` (commit `717d3da`): API routes register/login/logout, sesión cookie httpOnly 7d, protección de rutas (`/cuenta`, `/salas`), AuthForm + Navbar con logout, home con CTAs. Verificado en vivo (registro/login/protección). **22 tests ✅** · lint ✅ · build ✅ | P4.3 Salas y mensajes (tiempo real) + subir repo a GitHub + deploy Railway |
| 2026-08-12 | **P4.3 Chat: salas y tiempo real completado** | Repo `chat` subido a GitHub (`AsherAST/chat`). Salas y mensajes en vivo (commit `93ffcb6`): DAL `rooms.ts`, auth de sockets por cookie JWT, eventos `room:join`/`room:leave`/`message:send` con persistencia y emisión `message:new`, cliente `RoomChat` con ack, páginas `/salas` + `/salas/[id]` (historial + 404), Server Action `createRoomAction`. Fix bug dev: `dotenv` en `server.ts` (custom server no veía `.env`) + `db.ts` prefiere `DATABASE_URL_UNPOOLED`. Verificado con **2 clientes Socket.io** conversando en tiempo real + mensaje persistido ✅. **29 tests ✅** · lint ✅ · build ✅. Deploy en Suga probado (HTTP 200) pero **en pausa**: Suga no alcanza Neon (`databasenotreachable`), Koyeb rechaza altas, Railway cobra | P4.4 "escribiendo…" + presencia, retomar deploy, screenshot Dashboard |
| 2026-08-12 | **P4.4 Chat: escribiendo… + presencia completado** | Tiempo real avanzado en `chat/` (commit `7e9299b`): presencia por sala (`presence:update` al unirse/salir/desconectarse, multi-socket por usuario) e indicador "escribiendo…" (`typing:start`/`typing:stop` emitidos solo a los demás). Cliente `RoomChat` con avatares + "N en línea" y texto de escritura (throttle 1.5 s + auto-stop 3 s). Verificado en vivo: ambos clientes ven "2 en línea", typing llega y se detiene, al salir queda "1 en línea". **33 tests ✅** · lint ✅ · build ✅ | P4.5 Cierre (README, CI, deploy, screenshot, portafolio) + screenshot Dashboard |
| 2026-08-12 | **P4.5 Chat: cierre completado** | **README real** del proyecto + **CI en GitHub Actions** (`.github/workflows/ci.yml`, commit `a1fe80c`): lint + tests + build, **run #1 exitoso ✅**. **Chat en tiempo real publicado en el portafolio** (`content.ts` ES/EN, 4º proyecto, con link al código). **33 tests ✅** · lint ✅ · build ✅. **Pendientes**: deploy (Suga→Neon pausado), screenshots del chat y del Dashboard (falta `sudo apt-get install libnspr4 libnss3` para Chromium) | P4 terminado → retomar deploy + screenshots pendientes |
